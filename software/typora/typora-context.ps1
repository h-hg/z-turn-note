# custom
$reg_file_ass = "HKEY_CLASSES_ROOT\.md"
$reg_md_config_name = "mdfile"
$menu_name = "Open with &Typora"
...
if($choice -eq "1") {
  $exe_relative_path = "apps\typora\current\typora.exe"
  ...
  # associate the file
  reg add "$reg_file_ass" /ve /d "$reg_md_config_name" /f
  reg add "HKEY_CLASSES_ROOT\$reg_md_config_name" /ve /d "Markdown File" /f
  reg add "HKEY_CLASSES_ROOT\$reg_md_config_name\DefaultIcon" /ve /t REG_EXPAND_SZ /d "$exe_path" /f
  reg add "HKEY_CLASSES_ROOT\$reg_md_config_name\Shell\Open\Command" /ve /t REG_EXPAND_SZ /d "`\`"$exe_path`\`" `\`"%1`\`"" /f

  # directory menu
  reg add "$reg_dir_menu\$menu_name" /v "Icon" /t REG_EXPAND_SZ /d "$exe_path" /f
  reg add "$reg_dir_menu\$menu_name\command" /ve /t REG_EXPAND_SZ /d "`\`"$exe_path`\`" `\`"%1`\`"" /f

  # directory background menu
  reg add "$reg_dir_bg_menu\$menu_name" /v "Icon" /t REG_EXPAND_SZ /d "$exe_path" /f
  reg add "$reg_dir_bg_menu\$menu_name\command" /ve /t REG_EXPAND_SZ /d "`\`"$exe_path`\`" `\`"%V`\`"" /f
} elseif($choice -eq "2") {
  # associate the file
  reg delete "$reg_file_ass" /f
  reg deletee "HKEY_CLASSES_ROOT\$reg_md_config_name" /f
  # directory menu
