# custom
$menu_name = "Open with &Code"

# Registry variables
$reg_all_file_menu = "HKEY_LOCAL_MACHINE\Software\Classes\*\shell"
...
if($choice -eq "1") {
  $exe_relative_path = "apps\vscode\current\Code.exe"
  ...
  # all file menu
  reg add "$reg_all_file_menu\$menu_name" /v "Icon" /t REG_EXPAND_SZ /d "$exe_path" /f
  reg add "$reg_all_file_menu\$menu_name\command" /ve /t REG_EXPAND_SZ /d "`\`"$exe_path`\`" `\`"%1`\`"" /f
  ...
  # all file menu
  reg delete "$reg_all_file_menu\$menu_name" /f
